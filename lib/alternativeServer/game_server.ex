defmodule AlternativeServer.Game.GameServer do
  ...
  defp update_state_for_action(state, user_id, :set_position, %{
    ...
    # Fieldsにも配置情報を更新 (set_posの位置にカードデータを配置)
    new_fields =
      Map.update!(state.fields, user_id, fn field ->
        Map.put(field, set_pos, %{
          card_id: card_data["card_id"],
          hp: card_data["hp"] || 100,
          name: card_data["name"] || "Unknown Card",
          attack: card_data["attack"] || 0,
          guard: card_data["guard"] || 0,
          speed: card_data["speed"] || 1,
          range: card_data["range"] || 1,
          status: "set"
        })
      end)
      ...
      :position_check ->
        if all_players_ready?(state) do
          ...
          if has_revival do
            ...
            broadcast(state.room_id, "phase_changed:set", %{status: true})  #クライアントはこれが取得できてない
            new_state = reset_ready_status(state)
            {:noreply, %{new_state | phase: :card_select_check}}
          end
        else
  ...
  defp summon_set_cards(state) do
    ...
    events =
      for {player_id, field} <- state.fields,
        ...
        Logger.info(
          "[#{state.room_id}] Summoned card #{card_data.card_id} at position #{position} for player #{player_id}"
        )

        summon_payload = %{
          user_id: player_id,
          position: position,
          card_id: Map.get(card_data, :card_id) || 1,
          turn: state.turn
        }
    ...
    # 送信前にstateをログ出力する
    Logger.info("[#{state.room_id}] State before broadcast: #{inspect(state)}")
